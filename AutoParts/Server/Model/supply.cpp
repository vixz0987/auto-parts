...
Supply::Supply(QObject *parent)
    : QObject(parent), m_supplyId(0), m_quantity(0), m_priceChangeId(0)
{}

Supply::Supply(int supplyId, const QDate &supplyDate, int quantity, int priceChangeId)
    : QObject(nullptr), m_supplyId(supplyId), m_supplyDate(supplyDate),
    m_quantity(quantity), m_priceChangeId(priceChangeId)
{}
...
bool Supply::save()
{
    ...
    QSqlQuery query(db);
    query.prepare("INSERT INTO auto_parts.supplies (supply_date, quantity, price_change_id) "
                  "VALUES (:supply_date, :quantity, :price_change_id) RETURNING supply_id");
...
bool Supply::update()
{
    ...
    query.prepare("UPDATE auto_parts.supplies SET supply_date = :supply_date, "
                  "quantity = :quantity, price_change_id = :price_change_id "
                  "WHERE supply_id = :id");
    query.bindValue(":supply_date", m_supplyDate);
    query.bindValue(":quantity", m_quantity);
    query.bindValue(":price_change_id", m_priceChangeId);
...
Supply* Supply::loadById(int supplyId)
{
    QSqlDatabase db = Database::instance()->getDb();
    QSqlQuery query(db);
    query.prepare("SELECT supply_id, supply_date, quantity, price_change_id "
...
QList<Supply*> Supply::loadAll()
{
    ...
    QSqlDatabase db = Database::instance()->getDb();
    QSqlQuery query(db);
    query.exec("SELECT supply_id, supply_date, quantity, price_change_id "
...
// метод loadBySupplier полностью удалён